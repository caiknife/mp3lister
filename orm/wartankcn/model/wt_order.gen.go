...
type WtOrder struct {
	...
	OrderStatus int32     `gorm:"column:order_status;type:tinyint(4);not null;comment:订单类型" json:"order_status"`                          // 订单类型
	ProductID   string    `gorm:"column:product_id;type:varchar(64);not null;comment:产品ID" json:"product_id"`                             // 产品ID
	PayType     int32     `gorm:"column:pay_type;type:tinyint(4);not null;comment:支付类型" json:"pay_type"`                                  // 支付类型
	...
	Version     int64     `gorm:"column:version;type:bigint(20);comment:数据版本" json:"version"`                                             // 数据版本
}
